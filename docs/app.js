...
function buildFilters(){
  ...
  // Helper: rebuild a <select> keeping a blank "All ..." option first
  function rebuildSelect(sel, blankLabel, values){
    sel.innerHTML = "";
    ...
    sel.value = ""; // force neutral default (prevents "0 rows" from stale selection)
  }

  const areas = uniq(items.map(x => x.area).filter(Boolean)).sort();
  rebuildSelect(areaSel, "All areas", areas);

  const types = uniq(items.map(x => x.category).filter(Boolean)).sort();
  rebuildSelect(typeSel, "All types", types);

  function populateZones(){
    const area = areaSel.value;
    const zoneValues = uniq(items
      .filter(x => !area || x.area === area)
      .map(x => x.zone)
      .filter(Boolean)
    ).sort();

    // keep current selection only if still valid
    const prev = zoneSel.value;
    rebuildSelect(zoneSel, "All zones", zoneValues);
    if(prev && zoneValues.includes(prev)){
      zoneSel.value = prev;
    }
  }
  ...
  // When Area changes, refresh Zone list (zones depend on area)
  areaSel.addEventListener("change", () => {
    populateZones();
    applyExternalFilters();
  });

  // If the browser restores old form values from cache, force them blank on load
  areaSel.value = "";
  zoneSel.value = "";
  typeSel.value = "";
}
...
function applyExternalFilters(){
  ...
  // Safety: if a zone is selected that doesn't exist for the chosen area, ignore it
  if(area && zone){
    const validZones = new Set(items.filter(x => x.area === area).map(x => x.zone));
    if(!validZones.has(zone)) zone = "";
  }
